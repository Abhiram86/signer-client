...
export default function PDFView({ preDoc, simpleMode = false }: PDFViewProps) {
  ...
  useEffect(() => {
    ...
    const fetchAndProcessDocs = async () => {
      setIsLoading(true);
      if (preDoc && preDoc.length > 0) {
        const processedDocs: ProcessedDoc[] = preDoc.map((doc) => ({
          ...doc,
          isProcessing: false,
        }));
        setDocs(processedDocs);

        const processingPromises = preDoc.map((doc) => processDocument(doc));
        ...
        try {
          ...
          if (res.error) {
            ...
            const initialDocs: ProcessedDoc[] | undefined = res.docs?.map(
              (doc) => ({
                ...doc,
                isProcessing: true,
              })
            );
            initialDocs && setDocs(initialDocs);

            const processingPromises = res.docs?.map((doc) =>
              processDocument(doc)
            );
  ...
  const handleSignatureDrag = useCallback(
    (index: number, newX: number, newY: number) => {
      setSignatures((prev) =>
        prev.map((sig, i) => (i === index ? { ...sig, x: newX, y: newY } : sig))
      );
    },
    []
  );
        ...
        prev.map((sig, i) =>
          i === index
            ? {
                ...
                height: Math.max(0.03, newHeight),
              }
            : sig
        )
      );
    ...
    <div className="space-y-8">
      ...
      {!simpleMode && (
        <div className="bg-zinc-800 p-4 rounded-lg">
          <h3 className="text-zinc-100 font-medium mb-3">Signature Controls</h3>
